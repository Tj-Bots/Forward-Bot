...
async def run(bot, message):
    ...
    if len(channels) > 1:
       ...
       buttons.append([KeyboardButton("ביטול")]) 
    ...
    if fromid.text and not fromid.forward_date:
        ...
        if last_msg_id == None:
           return await message.reply_text("**ייתכן שזוהי הודעה מועברת מקבוצה שנשלחה על ידי מנהל אנונימי. במקום זאת, אנא שלח את הקישור להודעה האחרונה מהקבוצה.**")
    else:
  ...
  #  except ChannelInvalid:
    ...
    buttons = [[
        InlineKeyboardButton('כן', callback_data=f"start_public_{forward_id}"),
        InlineKeyboardButton('לא', callback_data="close_btn")
    ]]
